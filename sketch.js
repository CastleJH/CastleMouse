...
function draw() {
  ...
  fill(255);
  strokeWeight(2);
  ellipse(windowWidth/2, windowHeight/8, windowHeight/13, windowHeight/13);
  ...
  //changing line's color to white when moon touches it
  for (i = 0 ; i<n ; i++) {
    if (dist(rainX[i], rainY[i], x+(windowWidth/2), windowHeight/8) < windowHeight/26 || dist(rainX[i], rainY[i]+windowHeight/9, x+(windowWidth/2), windowHeight/8) < windowHeight/26) {
      rainColor[i] = color(255);
    }
    stroke(rainColor[i]) ;
    line(rainX[i], rainY[i], rainX[i], rainY[i]+windowHeight/9);
  }
}
